...
Our model is very similar to the one that ChatGBT produces, but some of the significant differences are:
- The model generated by ChatGBT does not assign primary key indices like we do for each of our tables
- We do not implement a "user" class like ChatGBT does that references the Student and Instructor Classes. We felt it would be reduntant to have that class because we can just differentiate accounts by Student or Instructor class
- We also do not reference the Instructor ID in the QRCode class because the instructor is already embedded in the Course ID reference
- We also set the email field to be unique because it is both unlikely and inconvenient for different instructors to have the same email. 
